  ...
  ({ className, size, children, Icon, icon, dotClassName, ...props }, ref) => {
    ...
    return (
      <button
        ...
        {...props}
      >
        <span className={cn('relative shrink-0 rounded-full flex items-center justify-center bg-[var(--primary-general)] transition-all duration-500 w-2.5 h-2.5', dotExpanded, dotClassName)}>
          <span className="text-[var(--primary2)] flex items-center justify-center opacity-0 scale-50 transition-all duration-300 group-hover:opacity-100 group-hover:scale-100 group-data-[touched=true]:opacity-100 group-data-[touched=true]:scale-100">
            {resolvedIcon}
          </span>
        </span>
        <span className="relative z-10 tracking-wide text-[var(--primary-general)] transition-transform duration-500 group-hover:translate-x-1.25 group-data-[touched=true]:translate-x-1.25">
          {children}
        </span>
      </button>
    );
